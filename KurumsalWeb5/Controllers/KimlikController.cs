...
namespace KurumsalWeb5.Controllers
{
    public class KimlikController : Controller
    {
        ...
        {
            var kimlik = db.Kimlik.Where(p => p.KimlikId == id).SingleOrDefault();
            return View(kimlik);
        }
        ...
        public ActionResult Edit(int id, Kimlik kimlik,HttpPostedFileBase LogoURL)
        {
            ...
            {
                var k = db.Kimlik.Where(p => p.KimlikId == id).SingleOrDefault();
                if (LogoURL != null)
                {
                    ...
                    string logonName = LogoURL.FileName;
                    img.Resize(300, 200);
                    img.Save("~/Uploads/Kimlik/" + logonName);
                    k.LogoURL = "/Uploads/Kimlik/" + logonName;
                }
